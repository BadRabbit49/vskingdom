...
namespace VSKingdom {
	public class AiTaskSentryWaters : AiTaskBase {
		...
		protected Vec3d curTargetPos = new Vec3d();
		protected BlockPos pos = new BlockPos(Dimensions.NormalWorld);

		public override bool ShouldExecute() {
			if (!entity.Swimming || entity.World.Rand.NextDouble() > 0.04f) {
				return false;
			}
			curTargetPos.Y = entity.ServerPos.Y;
			int tries = 6;
			...
			while (tries-- > 0) {
				...
				if ((blockBoxes == null || blockBoxes.Max((cuboid) => cuboid.Y2) <= 1f) && (belowBoxes != null && belowBoxes.Length > 0)) {
					curTargetPos.Set(pos.X + 0.5, pos.Y + 1, pos.Z + 0.5);
					return true;
				}
			}
			return false;
