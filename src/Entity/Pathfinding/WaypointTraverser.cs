...
namespace VSKingdom {
	public class SoldierWaypointsTraverser : PathTraverserBase {
		...
		public override bool NavigateTo(Vec3d target, float movingSpeed, float targetDistance, Action OnGoalReached, Action OnStuck, bool giveUpWhenNoPath = false, int searchDepth = 999, int mhdistanceTolerance = 0) {
			...
			float stepHeight = bh is null ? 1.2f : bh.stepHeight;
			...
			// Find alternate path if not traverserable from starting block position.
			if (!entity.World.BlockAccessor.IsNotTraversable(startBlockPos)) {
				waypoints = soldierAstar.FindPathAsWaypoints(startBlockPos, target.AsBlockPos, canFallDamage ? 8 : 4, stepHeight, entity.CollisionBox, searchDepth);
			}

			// Abandon if no waypoints, no path found.
			...
			// Now add a waypoint at the target.
			waypoints.Add(target);

			// Return false if there are is no path AND set to give up.
		...
		public override void OnGameTick(float dt) {
			...
			// If it takes more than 2 seconds to reach next waypoint (waypoints are always 1 block apart).
			bool stuck =
			...
			EntityControls controls = entity.MountedOn is null ? entity.Controls : entity.MountedOn.Controls;
		...
		bool IsNearTarget(int waypointOffset, ref bool nearHorizontally) {
			if (waypoints.Count - 1 < waypointToReachIndex + waypointOffset) {
				...
				// One block above or half a block below is fine.
